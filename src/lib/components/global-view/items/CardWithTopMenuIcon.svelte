...
<button class="p-2 rounded-lg shadow-lg cursor-pointer" on:click|stopPropagation={() => playSongZene(musicData, list)}>
  ...
  <p class="text-white urbanist-semibold mt-3 text-base text-center w-full line-clamp-2">{musicData.name}</p>
  <p class="text-white urbanist-thin mt-1 text-center w-full line-clamp-2">{musicData.artists}</p>
</button>