  ...
  <div class="w-full h-full fixed z-50 top-0 right-0 left-0 bg-gray-600 bg-opacity-80 flex justify-center items-center">
    <div class="p-4 w-full max-w-md max-h-full">
      <div class="relative bg-maincolor rounded-lg shadow dark:bg-gray-700">
        <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparentrounded-lg text-sm size-8" on:click|stopPropagation={() => (showDialog = false)}>
        ...
        <div class="p-4 md:p-5 text-center">
          ...
          <button data-modal-hide="popup-modal" type="button" class="text-black bg-white font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2" on:click|stopPropagation={() => (showDialog = false)}>Continue</button>
          <button class="text-gray-500 bg-white hover:bg-gray-100 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5" on:click|stopPropagation={() => window.location.reload()}>Refresh</button>
        </div>
      </div>
    </div>
  </div>
