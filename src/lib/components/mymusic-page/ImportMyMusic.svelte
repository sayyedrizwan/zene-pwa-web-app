...
<div>
  <div class="grid grid-cols-2 md:grid-cols-3 gap-4 p-4">
    {#each Array(3) as _, index (index)}
      <button class="w-full bg-maincolor rounded-lg py-12 md:py-20 px-4 flex flex-col justify-center" on:click={index == 0 ? startSpotify : index == 1 ? startYtMusic : startYtMusic}>
