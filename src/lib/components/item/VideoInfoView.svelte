...
<div class="flex-none py-6 px-3 first:pl-6 last:pr-6">
  {#if m.extra.trim() != ""}
    <button on:click={callYoutubeVideo} class="group flex flex-col items-center justify-center gap-3 w-64">
      <img class="w-full h-36 rounded-lg object-cover group-hover:p-4 hover-animation cursor-pointer" src={m.thumbnail.includes("ytimg") ? m.thumbnail.toString() : `https://i.ytimg.com/vi/${m.extra}/maxresdefault.jpg`} alt={m.name.toString()} referrerpolicy="no-referrer" />
