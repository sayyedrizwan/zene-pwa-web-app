...
<div class="flex-none py-6 px-3 first:pl-6 last:pr-6">
  {#if m.extra.trim() != ""}
    <button on:click={callYoutubeVideo} class="flex flex-col items-center justify-center gap-3 w-64 hover:w-56 hover:m-4 hover-animation cursor-pointer">
      <img class="w-full rounded-lg" src={m.thumbnail.includes("ytimg") ? m.thumbnail.toString() : `https://i.ytimg.com/vi/${m.extra}/maxresdefault.jpg`} alt={m.name.toString()} referrerpolicy="no-referrer" />
