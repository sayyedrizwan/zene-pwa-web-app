...
<div class="flex-none py-6 px-3 first:pl-6 last:pr-6">
  <button on:click={managePageDestination} class="group flex flex-col items-center justify-center gap-3 {dynamicList ? `w-full` : `w-64`}">
    <img class="w-full max-h-64 object-cover rounded-lg group-hover:p-4 hover-animation cursor-pointer" src={m.thumbnail.toString()} alt={m.name.toString()} referrerpolicy="no-referrer" />
