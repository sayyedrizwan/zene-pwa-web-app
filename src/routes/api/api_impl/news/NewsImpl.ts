...
export class NewsImpl {
  ...
  async searchBing(search: string, img: string): Promise<FeedData[]> {
    ...
    info.window.document.querySelectorAll('.news-card.newsitem.cardcommon').forEach((items) => {
      ...
      if (time == 'undefined') time = String(items.outerHTML)?.textAfterKeyword('tabindex="0" aria-label="')?.textBeforeKeyword('ago">')

      if (thumbnail == null) thumbnail = items.querySelector('.image.right')?.querySelector('img')?.getAttribute('data-src')
