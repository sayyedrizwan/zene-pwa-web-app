...
export class LyricsImpl {
  async rentanadvertiser(q: MusicData | null): Promise<string | null> {
    try {
      ...
      const lyrics = String(responseLyrics.window.document.querySelector('span#ctl00_ContentPlaceHolder1_lbllyrics_simple')?.outerHTML)
        .textAfterKeyword('</h3>')
        ?.replaceAll('<br>', '.')
        ?.replaceAll('www.RentAnAdviser.com', '')
          ...
          json.hot_songs_preview.forEach((e: any) => {
            if (
              e.url
                .replaceAll('-', ' ')
                ?.toLowerCase()
                ?.includes(q?.artists?.textBeforeKeyword(',')?.textBeforeKeyword('&')?.toLowerCase() ?? '---') &&
              e.title?.toLowerCase()?.includes(q?.name?.toLowerCase() ?? '---')
            ) {
