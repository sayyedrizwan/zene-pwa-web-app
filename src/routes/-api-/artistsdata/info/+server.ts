...
import { ArtistsDataInfo } from "../../ApiService/model/ArtistsData.js";
    ...
    [1, 2, 3, 4, 5, 6].map(async (page) => {
      try {
        if (page == 1 || page == 2) {
          ...
          await Promise.all(
            lastFMTopSongs.map(async (songs) => {
          ...
          try {
            const radioList = (await YoutubeAPIService.instance.searchVideosPlaylist(`${name} greatest hits`)) ?? [];
