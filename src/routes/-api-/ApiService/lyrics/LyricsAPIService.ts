  ...
  async lyricsData(name: String, artists: String): Promise<MusicLyricsData | undefined> {
    try {
      ...
      root
        ...
        ?.querySelectorAll("a")
        ?.forEach((html) => {
          if (checkRentAnAdvisorLyricsChecks(name, artists, html?.text?.trim()) && url == "") url = html.getAttribute("href") ?? "";
        });
      ...
      const t = text.replaceAll("by RentAnAdviser.com", "🎶 🎵").replaceAll("by rentanadviser.com", "🎶 🎵").replaceAll("by RentAnAdviser.com", "🎶 🎵").replaceAll("www.RentAnAdviser.com", "🎶 🎵").replaceAll("By RentAnAdviser.com", "🎶 🎵").replaceAll("\r\n", "").replaceAll("</h3>", "");
