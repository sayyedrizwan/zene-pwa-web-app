...
<svelte:head>
  ...
  {:else}
    <title>{songInfo.name} radio on Zene a Music app</title>
    <meta name="description" content="Radio {songInfo.artists}" />
    <meta name="og:description" content="Radio {songInfo.artists}" />
    <meta property="og:title" content="{songInfo.name} radio on Zene a Music app" />
...
{#if songInfo?.id == undefined}
  <main class="grid min-h-full place-items-center px-6 py-24 sm:py-32 lg:px-8">
    <div class="text-center">
      <h2 class="text-center text-white text-3xl xl:text-5xl leading-snug poppins-semi-bold mt-6 mb-4">No Radio Found</h2>
...
{:else}
  <div class="flex flex-col justify-center items-center mt-24 mx-4 mb-72">
    <img src={songInfo.thumbnail.toString()} alt={songInfo.name.toString()} class="object-cover w-80" />
